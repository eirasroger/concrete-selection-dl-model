...
def pad_features(features_list):
    ...
    mask = torch.zeros(batch_size, max_len, dtype=torch.bool)  # boolean mask
...
# Usage:
batch_size = 3
# Assume scenarios_batch is defined, e.g.,
scenarios_batch = scenarios[:batch_size]

# Extract features (list of tensors)
features_list = [torch.tensor(scenario['features']) for scenario in scenarios_batch]

# Pad features and get mask
dummy_input, dummy_mask = pad_features(features_list)

# Now you can pass dummy_input and dummy_mask to your model or test function
output = model(dummy_input, dummy_mask)

# Now you can call your test_loaded_model with:
test_loaded_model(dummy_input=dummy_input, dummy_mask=dummy_mask)
""" 
