...
SCENARIOS_FILE = os.path.join(output_dir, "gwp_control_scenarios.json")
LABELS_FILE = os.path.join(output_dir, "gwp_control_labels.json")
...
def get_ideal_attributes():
    ...
    # Health score varies between 4 (Silver), 5 (Gold), 6 (Platinum)
    ...
    # Water & Biodiversity (Ideal = Low impact)
...
def generate_control_dataset(num_scenarios):
    ...
    for i in range(1, num_scenarios + 1):
        ...
        # ──────── GWP MAPPING ────────
        # Evenly spaced GWP values
        base_gwps = np.linspace(GWP_MIN, GWP_MAX, n_alts)
        np.random.shuffle(base_gwps)
        
        alternatives = []
        labels_for_case = []

        for idx, gwp_val in enumerate(base_gwps):
            # Add small noise to GWP input
            input_noise = random.uniform(-0.01, 0.01)
            final_gwp = max(GWP_MIN, min(GWP_MAX, gwp_val + input_noise))
            final_gwp = round(final_gwp, 3)
            ...
            alt = {
                ...
                "fu_haz": attrs["fu_haz"],
                "health": attrs["health"],
                "gwp": final_gwp, # CONTROL VARIABLE
                "wdp": attrs["wdp"],
                "fwu": attrs["fwu"],
                "b": attrs["b"],
                "c": attrs["c"],
            ...
            # ──────── CALCULATE LABEL ────────
            gwp_range = GWP_MAX - GWP_MIN
            score_range = PREF_MAX - PREF_MIN
            
            normalized_position = (GWP_MAX - final_gwp) / gwp_range
            ...
            labels_for_case.append({
                ...
                "pref": pref_score,
                "conf": 1.0,
                "reason": f"Control Case: GWP {final_gwp} results in base score {round(base_score,3)} + noise."
            })
...
if __name__ == "__main__":
    print(f"Generating {N_SCENARIOS} control scenarios...")
    ...
    for alt, lab in zip(s1['alternatives'], l1['labelled_alternatives']):
        print(f"  ID: {alt['id_prod']} | GWP: {alt['gwp']} -> Score: {lab['pref']}")
