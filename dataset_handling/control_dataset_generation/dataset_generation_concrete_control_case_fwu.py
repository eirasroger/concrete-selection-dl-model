...
SCENARIOS_FILE = os.path.join(output_dir, "fwu_control_scenarios.json")
LABELS_FILE = os.path.join(output_dir, "fwu_control_labels.json")
...
# FWU Range (m3 freshwater use per kg)
FWU_MIN = 0.0005
FWU_MAX = 0.005
...
def get_ideal_attributes():
    ...
    NOTE: FWU is excluded here as it is the control variable.
    """
    ...
    # WDP (Fixed at Ideal/Low)
    wdp = random.uniform(0.03, 0.05)

    # Biodiversity (Ideal = Low impact)
...
def generate_control_dataset(num_scenarios):
    ...
    for i in range(1, num_scenarios + 1):
        ...
        # ──────── FWU MAPPING ────────
        # Evenly spaced FWU values across the range
        base_fwus = np.linspace(FWU_MIN, FWU_MAX, n_alts)
        np.random.shuffle(base_fwus)
        
        alternatives = []
        labels_for_case = []

        for idx, fwu_val in enumerate(base_fwus):
            # Add small noise to FWU input
            # FWU values are very small, so noise must be very small (e.g., +/- 0.00005)
            input_noise = random.uniform(-0.00005, 0.00005)
            final_fwu = max(FWU_MIN, min(FWU_MAX, fwu_val + input_noise))
            final_fwu = round(final_fwu, 6) 

            # Get Base Ideal Attributes (including low GWP & WDP)
            ...
            alt = {
                ...
                "gwp": attrs["gwp"], 
                "wdp": attrs["wdp"], 
                "fwu": final_fwu,    # CONTROL VARIABLE
                "b": attrs["b"],
                "c": attrs["c"],
            ...
            # ──────── CALCULATE LABEL ────────
            # Lower FWU = Higher Score
            fwu_range = FWU_MAX - FWU_MIN
            score_range = PREF_MAX - PREF_MIN
            
            normalized_position = (FWU_MAX - final_fwu) / fwu_range
            ...
            labels_for_case.append({
                ...
                "pref": pref_score,
                "conf": 1.0,
                "reason": f"Control Case: FWU {final_fwu} results in base score {round(base_score,3)} + noise."
            })
...
if __name__ == "__main__":
    print(f"Generating {N_SCENARIOS} FWU control scenarios...")
    ...
    for alt, lab in zip(s1['alternatives'], l1['labelled_alternatives']):
        print(f"  ID: {alt['id_prod']} | FWU: {alt['fwu']} -> Score: {lab['pref']}")
